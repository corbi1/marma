			...
			request.onsuccess = function(event) {
				...
				if (cursor) {
					...
					if (progressEntry.done === false) {
						console.log(cursor);
						console.log("id " + progressEntry.id);
						if (cursor.key === 1){ // open the very first lesson (intro) directly if it is not done
							console.log('Redirecting to:', `lessons/lesson.html?id=${progressEntry.id}&nr=${progressEntry.order}`);
							window.location.href = `lessons/lesson.html?id=${progressEntry.id}&nr=${progressEntry.order}`;
						}
